...
namespace ClothLearn {
    ...
    public struct DistanceConstraintJob : IJobFor {
        ...
        public void Execute(int index) {
            var constriant = distanceConstriants[index];

            var p0 = predictPositions[constriant.vIndex0];
            var p1 = predictPositions[constriant.vIndex1];
            var m0 = masses[constriant.vIndex0];
            var m1 = masses[constriant.vIndex1];
            var distV = p1 - p0;
            var normal = math.normalize(distV);
            var length = math.length(distV);
            var err = length - constriant.restLength;
            float3 correct;
            if (err<0) {
            ...
            var totalM = m0 + m1;
            positionCorrects[constriant.vIndex0] += correct * di * m1 / totalM;
            positionCorrects[constriant.vIndex1] -= correct * di * m0 / totalM;
        }
    }
    ...
    [Unity.Burst.BurstCompile]
    public struct NormalCalculateJob : IJob {
        [ReadOnly]
